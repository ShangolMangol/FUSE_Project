...
This Python script measures the performance of writing and reading JPEG images from `/mnt` and `/storage` folders, with automatic cleanup functionality.
...
- **Comparison**: Compares performance between `/mnt` and `/storage` directories
...
- Write permissions to `/mnt` and `/storage` directories
...
```bash
python3 jpeg_performance_test.py /path/to/your/jpeg/folder
```

### Save Results to JSON

```bash
python3 jpeg_performance_test.py /path/to/your/jpeg/folder --output results.json
```

### Skip Cleanup (for debugging)

```bash
python3 jpeg_performance_test.py /path/to/your/jpeg/folder --no-cleanup
```

### Full Example

```bash
python3 jpeg_performance_test.py /home/user/images --output performance_results.json
```
...
3. **Performance Comparison**: Side-by-side comparison between `/mnt` and `/storage`
...
Source folder: /home/user/images
Found 15 JPEG files in /home/user/images

=== Testing MNT Directory ===
Processing 1/15: image1.jpg
  Write: 0.0234s (45.67 MB/s)
  Read:  0.0156s (68.45 MB/s)
...

MNT Directory Summary:
  Total files: 15
...
=== Cleaning up ===
Removed /mnt/jpeg_test
Removed /storage/jpeg_test
...
```json
{
  ...
  "source_folder": "/home/user/images",
  "tests": [
    {
      "directory": "/mnt/jpeg_test",
      "test_name": "MNT Directory",
      "files": [
        {
          "filename": "image1.jpg",
...
- **Permission errors**: Handles cases where `/mnt` or `/storage` are not writable
...
- The script creates temporary directories `/mnt/jpeg_test` and `/storage/jpeg_test`
